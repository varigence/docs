...
import SettingsHub from './_settings_hub.md';
...
**Core Business Concepts** are related to other ones through [**Natural Business Relationships**](./data-vault-concept-link) (Links) and are described using [**Context**](./data-vault-concept-satellite) (Satellites) entities.

Any Hub entity in Data Vault can therefore have multiple active relationships to other entities through [Links](./data-vault-concept-link).

Each Hub maintains a distinct list of [**Integration Keys**](./data-vault-concept-integration-keys) that represent unique instances of the identified concept.
...
In BimlFlex, this is supported through the [**Business Modeling**](../metadata-editors/business-modeling) feature. [**Business Modeling**](../metadata-editors/business-modeling) supports an interactive workshop-style modeling of the involved processes and terminology, so that the right business terms can be defined.

The resulting Business Entities can be mapped to Data Vault Hubs in the [**Object Editor**](../metadata-editors/object-editor) or the Data Vault [**Accelerator**](../metadata-editors/accelerator).
...
The easiest and quickest way to define a new Hub is to define one based on source metadata using the Data Vault [**Accelerator**](../metadata-editors/accelerator).
...
For more information, please refer to the [**Accelerator**](../metadata-editors/accelerator) documentation.
...
* The [*Append Hub*](../reference-documentation/settings/DvAppendHub) setting in the *Data Vault Naming* setting category contains the label which will be added to a new Hub **Object**, for example 'HUB'
* Whether the above label will be added as a `Prefix` or `Suffix` is managed in the [*Suffix or Prefix Object*](../reference-documentation/settings/SuffixOrPrefixObject) setting which is found in the *Naming* setting category
...
![Defining a new Hub Object](/img/bimlflex/hub-manual-create-object.png "Defining a new Hub Object")

Using the [**Object Editor**](../metadata-editors/object-editor), a new **Object** with the Object Type `Hub` can be created.
...
By default, a Hub Object contains at least two **Columns**: a [**Surrogate Key**](./data-vault-concept-integration-keys#surrogate-keys) and an **Integration Key**. Other columns can be either added or configured using the [**Configurations**](./data-vault-concept-hub#hub-column-configuration).
...
![Creating a Hub Mapping](/img/bimlflex/hub-manual-create-mapping.png "Creating a Hub Mapping")
...
This is controlled using the The [*Infer Link Hub*](../reference-documentation/settings/DvInferLinkHub) setting in the *Data Vault* setting category. By default, this will be disabled and BimlFlex will manage the order of load dependency to avoid Referential Integrity issues.
...
* [Load Date Time Stamp](./data-vault-best-practices#roweffectivefromdate)
* [Record Source](./data-vault-best-practices#record-source)
* [Audit Id](./data-vault-best-practices#audit-id)

Columns such as these are defined in the BimlFlex [**Configurations**](../metadata-editors/configuration-editor). The **Configurations** screen in the BimlFlex App enables modelers to define which **Columns** apply to certain **Object Types**. For a **Column** to be added to the Hub, the **Configuration** will need to be configured so that the *Hub Attribute* is set to be `Derived`.
...
The [*Use Hash Keys*](../reference-documentation/reference-documentation-settings-index) setting can be used to toggle between using a hashed or natural **Integration Key**. Integrate without a `Hash` which is very efficient for modern data warehouse solutions like [Snowflake](https://www.snowflake.com/) and [Azure Synapse](https://azure.microsoft.com/en-us/services/synapse-analytics/).
