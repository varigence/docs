...
This is an example where a Source Pipeline Extension Point is used to discard all incoming rows where the Business Key column value is empty. This was used in a scenario where a file was loaded with empty rows that needed to be discarded.

Any row with an empty Business Key is discarded.
...
<!-- Will discard any row with an empty value in the BusinessKey column and log the row count as an Exception (Type 8) in the BimlCatalog -->
...
<ConditionalSplit Name="CSPL - Discard Empty Rows">
    ...
    <OutputPaths>
        <OutputPath Name="Discard">
            <Expression><#=tgtTable.BusinessKeyName #> == &quot;&quot;</Expression>
        </OutputPath>
    </OutputPaths>
</ConditionalSplit>
