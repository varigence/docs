...
* Blob Storage Connection that are defined as Source System no longer require a Catalog to be defined. 
...
* Fixed a bug for SSIS deployment which caused a duplication of a RowSourceId in the output when setting a higher thread count than one.
* Fixed a bug where setting a NULL value replacement for hashing was not correctly applied to the Integration Key hash.
...
- Fixed a bug that caused some PSA code to only compared Staging/PSA on Source (Natural) Keys, not Business Keys.
...
- Fixed a bug where the Data Vault setting 'Use Where Exist' did not generate the proper NOT EXIST statements in some cases.
- Fixed a bug that placed duplicated Extension Point files in the wrong folder.
...
- Fixed a bug where a Staged Query object with a RowChangeType configuration set to 'derived' uses @@this as a literal in the select statement instead of the table name.
...
- Fixed a bug where multiple metadata imports with different naming conventions duplicated the Integration Key columns. Columns names are now replaced with the updated naming convention where re-importing.
...
- Fixed a bug where Blob Storage staging configurations for on-premise solutions adds Objects to the model, but without creating Staging and PSA Objects.
- Fixed a bug where setting the Auto Adjust Buffer Size did not work when using Visual Studio 2019.
- Fixed a bug related to the 'RowIsDeleted' configuration. This has been updated to use '0' (false) as default value, where the default used to be '1' (true).
...
- Fixed a bug where the Linked Service properties for an OLEDB SQL Server Connection were inverted.
- Fixed a bug that required the SSL property to be set for a Postgre SQL  connection string. This is not enforced anymore.
- Fixed a bug where the metadata import reports on a login error using a MySQL Connection, even though all credentials are provided. This was related to differences between MySQL versions, and is now correctly supported for current and recent versions of MySQL.
...
- Fixed a bug where the Account Key for an ADF Connection was incorrectly saved with an additional semicolon.
- Fixed a bug where 'DataFactoryName' was not being emitted to ARM templates.
- Fixed a bug where in ADF extract projects no load Stored Procedures were generated if the LinkedService used is for a Managed Instance.
