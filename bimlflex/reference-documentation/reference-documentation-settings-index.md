...
|[Database References](xref:bimlflex-reference-documentation-setting-AdditionalDatabaseReferences) |![Text Datatype](../static/svg/text.svg "Text Datatype")|Use this comma-delimited list to specify additional databases for matching that may not be defined as a connection when the `Use Database Reference` option is checked, applicable for Snowflake or SSDT.|
