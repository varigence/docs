...
|[Delta Detection in SQL](xref:bimlflex-app-reference-documentation-setting-PsaDeltaDetectionSql) |![Text Datatype](images/svg-icons/boolean.svg "Boolean Datatype")|Determines whether the delta detection applied when loading changes to the PSA uses a SQL procedure that runs on the Staging Area table, and not as part of the PSA pattern.|
