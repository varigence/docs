...
Objects in BimlFlex are the data stores that can be used as source, or target, for data logistics. Objects are managed in the [`Object Editor`](xref:objects).
...
|IsNotPersistent | When enabled, this will prevent a thread Staging Area (PSA) table or file from being created for this Object. This features defines an override that applies to an Object for a Connection that impacts the behaviour in the Persistent Staging Area (PSA). When enabled, the Object will not be stored in the PSA. In other words, setting this override to `Y` will exclude the column from being persisted.|
|ExcludeFromBuild | Entering Y into this column will determine that this Object will be built along with the rest of the solution when the BimlFlex solution is being compiled. Disabling this will not generate an output artifact for the Object, as it will be removed from the build process.|
...
|ExcludeFromValidation | Entering Y into this column will determine that this object’s metadata will be validated along with the rest of the BimlFlex solution. Disabling this will mean that the Object will be ignored by the validation engine. In some cases, this may be helpful when Objects have metadata that is generated dynamically in a downstream process that would otherwise cause validation issues, such as for some Point-In-Time or Bridge tables.|
...
|OverrideSql | Overrides the complete SQL statement used to query the Object. This must be a complete SQL statement. Adding an override here disregards any other SQL overrides, such as FROM SQL or WHERE SQL.|
