...
| Multi Active Row    | Data Vault only.  Assigns the column to be a multi-active value for the associated object.  Used to determine the grain of a Multi-Active Satellite.  An object can have only one column defined as multi-active. This columns must also be set to Primary Key together with the Integration Key |
