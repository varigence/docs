...
BimlFlex will automatically assign the permissions needed by the Azure Data Factory so it can access the required Secrets.
In addition, required Secrets will also be created and populated with a value of `xxx`.

> [!WARNING]
> The `AzureKeyVault` **Setting** along with an *AKV Linked Service* set to `BimlFlex Autogenerated KeyVault` should not be used after initial creation unless.
> This combination will destroy and recreate the Azure Key Vault, clearing out any custom permissions or Secrets in the process.
...
BimlFlex currently only support permission assignment and Secret creation when auto generating a Key Vault.
All Secrets inputted into BimlFlex will have to me manually created and populated.
...
### Azure Blob Stage Container Settings (Blob Container Configuration Only)
